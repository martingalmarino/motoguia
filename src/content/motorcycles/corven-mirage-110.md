---
id: corven-mirage-110
brandId: corven
name: Mirage 110
variant: AD
modelYear: 2026
market: AR
status: published
indexable: true
availability: available
availabilityCheckedAt: 2026-09-18
availabilitySourceIds:
  - corven-mirage
  - acara-siomaa-2026-08
category: cub
engineType: combustion
chineseHub: true
brandOriginCountry: AR
manufacturingCountry: null
assemblyCountry: null
importer: Corven Motos
price: null
engine:
  displacementCc: 110
  powerKw: 5.516241
  declaredPowerOriginalUnit: hp
  declaredPowerOriginalValue: 7.5
  torqueNm: null
  cooling: null
  fuelSystem: null
  transmission: 4 velocidades, embrague semiautomático
  gears: 4
chassis:
  brakesFront: Disco hidráulico
  brakesRear: Tambor
  abs: null
  cbs: null
  suspensionFront: null
  suspensionRear: null
  tireFront: null
  tireRear: null
dimensions:
  lengthMm: null
  widthMm: null
  heightMm: null
  wheelbaseMm: null
  seatHeightMm: null
  weightKg: 99
  weightBasis: la ficha escribe en seco/orden de marcha
fuel: null
electric: null
editorial:
  summary: Cub Corven con potencia aproximada (~7,5 HP) según la propia ficha.
  suitableFor:
    - Uso urbano
  strengths:
    - Semiautomática declarada
    - Disco delantero en AD
  limitations:
    - La potencia está marcada como aproximación
    - Tanque no informado
  ownershipNotes: "La tilde de aproximación se conserva: no se redondea como cifra exacta."
images:
  - path: /images/motos/corven-mirage-110.jpg
    alt: Corven Mirage 110 AD blanca, foto de catálogo de Corven Motos
    source: Corven Motos
    rightsBasis: manufacturer-catalog
    credit: Foto del catálogo de Corven Motos.
sourceIds:
  - corven-mirage
  - acara-siomaa-2026-08
relatedModelIds:
  - corven-energy-110
  - gilera-smash
relatedGuideIds:
  - primera-moto-argentina
accessoryCategoryIds:
  - helmets
rankingReportNames:
  - CORVEN MIRAGE 110
publishedAt: 2026-09-18
reviewedAt: 2026-09-18
seoTitle: Corven Mirage 110 ficha Argentina
seoDescription: Datos de la Corven Mirage 110 AD, incluyendo la potencia aproximada que declara la propia marca.
licenceHint: 110 cc, clase A 1.2 según ANSV.
warranty: null
---

CORVEN MIRAGE 110 suma 6.948 patentamientos en el acumulado 2026. La ficha AD indica 110 cc, ~7,5 HP a 7.000 rpm, 4 velocidades con embrague semiautomático, disco hidráulico delantero, tambor trasero y 99 kg. El tanque aparece vacío en la ficha revisada. La potencia se trata como valor aproximado porque así lo escribe Corven.
