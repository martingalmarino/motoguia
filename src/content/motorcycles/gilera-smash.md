---
id: gilera-smash
brandId: gilera
name: Smash
variant: Informe ACARA agrupado; ficha citada Full II
modelYear: 2026
market: AR
status: published
indexable: true
availability: available
availabilityCheckedAt: 2026-09-18
availabilitySourceIds:
  - gilera-smash
  - acara-siomaa-2026-08
category: cub
engineType: combustion
chineseHub: true
brandOriginCountry: null
manufacturingCountry: null
assemblyCountry: null
importer: Gilera Motors Argentina
price: null
engine:
  displacementCc: 107
  powerKw: 5.295591
  declaredPowerOriginalUnit: hp
  declaredPowerOriginalValue: 7.2
  torqueNm: null
  cooling: null
  fuelSystem: null
  transmission: Semiautomática 4 marchas
  gears: 4
chassis:
  brakesFront: Disco (Full II)
  brakesRear: Disco (Full II)
  abs: null
  cbs: true
  suspensionFront: null
  suspensionRear: null
  tireFront: null
  tireRear: null
dimensions:
  lengthMm: null
  widthMm: null
  heightMm: 1070
  wheelbaseMm: null
  seatHeightMm: null
  weightKg: 87
  weightBasis: en seco, Smash Full II
fuel:
  tankLiters: 3.5
  consumptionL100km: null
  measurementBasis: null
  sourceIds:
    - gilera-smash
electric: null
editorial:
  summary: Segunda en patentamientos 2026. ACARA agrupa Smash; la ficha citada es la Full II.
  suitableFor:
    - Uso urbano diario
    - Cub 110 semiautomática
  strengths:
    - Volumen de patentamientos muy alto
    - CBS y doble disco en Full II
  limitations:
    - Sin precio de lista oficial hallado
    - El ranking no distingue versiones
  ownershipNotes: Garantía Gilera declarada 12 meses o 12.000 km.
images:
  - path: /images/motos/gilera-smash.png
    alt: Gilera Smash Full II roja, foto de catálogo de Gilera Argentina
    source: Gilera Motors Argentina
    rightsBasis: manufacturer-catalog
    credit: Foto del catálogo de Gilera Argentina.
sourceIds:
  - gilera-smash
  - gilera-ar
  - acara-siomaa-2026-08
relatedModelIds:
  - honda-wave-110s
  - corven-energy-110
  - zanella-zb-110
relatedGuideIds:
  - moto-110-125-150
  - garantia-repuestos-servicio
accessoryCategoryIds:
  - helmets
  - security
rankingReportNames:
  - GILERA SMASH
publishedAt: 2026-09-18
reviewedAt: 2026-09-18
seoTitle: Gilera Smash ficha técnica en Argentina
seoDescription: Qué cubre la ficha Smash Full II y cómo leer el ranking ACARA de la Gilera Smash en Argentina.
licenceHint: Hasta 150 cc, clase A 1.2 según ANSV.
warranty: 12 meses o 12.000 km, según Gilera.
---

En el acumulado ACARA enero-agosto 2026, GILERA SMASH suma 51.687 patentamientos y queda segunda. El informe no abre esa fila por versión.

La ficha usada acá es Smash Full II: 107 cm³, 7,2 hp a 7.500 r/min, semiautomática de 4 marchas, CBS, discos adelante y atrás, tanque de 3,5 litros y 87 kg en seco. Los 1.070 mm de la ficha son alto total, no altura de asiento.

No hay precio de lista en la página revisada. La conversión a kW usa caballo métrico (0,7355 kW/hp) y se declara para no mezclar unidades.
